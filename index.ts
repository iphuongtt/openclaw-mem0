...
class PlatformProvider implements Mem0Provider {
  ...
  constructor(
    private readonly apiKeyOrRef: string | SecretRef,
  ...
  private async _init(): Promise<void> {
    const apiKey = isSecretRef(this.apiKeyOrRef)
      ? await resolveSecretRef(this.apiKeyOrRef, this.api!)
      : this.apiKeyOrRef;
...
class OSSProvider implements Mem0Provider {
  ...
  private async _init(): Promise<void> {
    ...
    // Resolve any secret refs inside ossConfig (e.g. embedder.config.apiKey)
    let resolvedOss = this.ossConfig;
    if (resolvedOss && this.api) {
      resolvedOss = await resolveDeep(resolvedOss as unknown as Record<string, unknown>, this.api) as unknown as Mem0Config["oss"];
    }

    const config: Record<string, unknown> = { version: "v1.1" };

    if (resolvedOss?.embedder) config.embedder = resolvedOss.embedder;
    if (resolvedOss?.vectorStore)
      config.vectorStore = resolvedOss.vectorStore;
    if (resolvedOss?.llm) config.llm = resolvedOss.llm;

    if (resolvedOss?.historyDbPath) {
      const dbPath = this.resolvePath
        ? this.resolvePath(resolvedOss.historyDbPath)
        : resolvedOss.historyDbPath;
...
async function resolveSecretRef(ref: SecretRef, api: OpenClawPluginApi): Promise<string> {
  // Use openclaw's built-in secret resolution if available
  if (typeof (api as any).resolveSecret === "function") {
    return (api as any).resolveSecret(ref);
  }
  // Fallback: resolve via exec using the secrets providers in config
  const { spawnSync } = await import("child_process" as any);
  const proc = (globalThis as any).process as { env: Record<string, string | undefined> };
  const secrets = (api as any).config?.secrets?.providers?.[ref.provider];
  if (!secrets) {
    throw new Error(`Secret provider "${ref.provider}" not found in config`);
  }
  const { command, args = [], passEnv = [] } = secrets;
  const env: Record<string, string> = { ...proc.env } as Record<string, string>;
  for (const key of passEnv) {
    if (proc.env[key]) env[key] = proc.env[key]!;
  }
  const result = spawnSync(command, args, { env, encoding: "utf8" });
  if (result.error) throw result.error;
  if (result.status !== 0) throw new Error(`Secret provider "${ref.provider}" exited with code ${result.status}: ${result.stderr}`);
  return (result.stdout as string).trim();
}

async function resolveValue(value: unknown, api: OpenClawPluginApi): Promise<unknown> {
  if (typeof value === "string") return resolveEnvVars(value);
  if (isSecretRef(value)) return resolveSecretRef(value, api);
  if (value && typeof value === "object" && !Array.isArray(value)) {
    return resolveDeep(value as Record<string, unknown>, api);
  }
  return value;
}

async function resolveDeep(obj: Record<string, unknown>, api: OpenClawPluginApi): Promise<Record<string, unknown>> {
  const result: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(obj)) {
    result[key] = await resolveValue(value, api);
  }
  return result;
}
...
const mem0ConfigSchema = {
  parse(value: unknown): Mem0Config {
    ...
    // Platform mode requires apiKey (string or secret ref)
    if (mode === "platform") {
      if (!cfg.apiKey || (typeof cfg.apiKey !== "string" && !isSecretRef(cfg.apiKey))) {
        throw new Error(
    ...
    // Resolve env vars in oss config (secret refs resolved later in register() with api)
...
function createProvider(
  ...
  if (cfg.mode === "open-source") {
    return new OSSProvider(cfg.oss, cfg.customPrompt, (p) =>
      api.resolvePath(p), api,
    );
  }

  const apiKeyOrRef: string | SecretRef = isSecretRef(rawApiKey)
    ? rawApiKey
    : cfg.apiKey!;
  return new PlatformProvider(apiKeyOrRef, cfg.orgId, cfg.projectId, api);
}
...
const memoryPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    const rawApiKey = (api.pluginConfig as Record<string, unknown>).apiKey;

    const provider = createProvider(cfg, api, rawApiKey);
